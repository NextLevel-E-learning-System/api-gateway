...
function extractToken(req: Request): string | null {
  ...
  const isRefreshRoute = req.method === 'POST' && path === '/auth/v1/refresh'
  ...
  // 2. Cookie - usar refreshToken para /auth/v1/refresh, accessToken para outras rotas
  if (req.headers.cookie) {
    try {
      ...
      // Para rota de refresh, buscar refreshToken
      if (isRefreshRoute && cookies.refreshToken) {
        console.log('[extractToken] Token encontrado no cookie refreshToken (refresh route)')
        return cookies.refreshToken
      }

      // Para outras rotas, buscar accessToken
      if (!isRefreshRoute && cookies.accessToken) {
